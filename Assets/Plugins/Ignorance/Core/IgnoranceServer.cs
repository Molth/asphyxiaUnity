        ...
        private void ThreadWorker(Object parameters)
        {
            ...
            using (serverENetHost = new Host())
            {
                ...
                try
                {
                    ...
                    servicePeer = serverENetHost.Connect(serviceAddress, setupInfo.Channels);
                ...
                while (!CeaseOperation)
                {
                    ...
                    while (Outgoing.TryDequeue(out IgnoranceOutgoingPacket outgoingPacket))
                    {
                        ...
                        {
                            var ret = serverPeerArray[outgoingPacket.NativePeerId].Send(outgoingPacket.Channel, ref outgoingPacket.Payload);
                    ...
                    while (!pollComplete)
                    {
                        ...
                        switch (serverENetEvent.Type)
                        {
                            ...
                            case EventType.Receive:
                                ...
                                if (incomingPeer.ID == servicePeer.ID)
                                {
                                    var packet = serverENetEvent.Packet;
                                    var endPoint = new ENetIPEndPoint(packet, 1);
                                    ...
                                    {
                                        var address = endPoint.CreateAddress();
                                        var peer = serverENetHost.Connect(address);
                                        if (!serverPeerArray[peer.ID].IsSet)
